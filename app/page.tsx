...
import { LayoutDashboard, Receipt, Target, Settings, Menu, Moon, Sun, ChevronLeft } from "lucide-react"
    ...
    <div className="min-h-screen bg-background pb-20 md:pb-0 transition-all duration-300">
      ...
      >
        <div className="flex h-16 items-center justify-between border-b px-4">
          {!sidebarCollapsed && <h1 className="text-xl font-bold text-primary">Finanças Will</h1>}
          <Button
            variant="ghost"
            size="icon"
        ...
        <nav className="flex-1 space-y-1 p-3">
          <button
            ...
            className={`flex w-full items-center ${sidebarCollapsed ? "justify-center" : "gap-3"} rounded-lg px-4 py-3 text-left transition-all hover:scale-105 ${
              activeTab === "dashboard"
        ...
        <div className="border-t p-3">
          <Button
            variant="outline"
            className={`w-full ${sidebarCollapsed ? "px-0" : "justify-start gap-3"}`}
            onClick={toggleTheme}
            title={sidebarCollapsed ? (theme === "dark" ? "Tema Claro" : "Tema Escuro") : ""}
          >
      ...
      <main className={`min-h-screen transition-all duration-300 ${sidebarCollapsed ? "md:pl-20" : "md:pl-64"}`}>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 border-t bg-card/95 backdrop-blur-sm md:hidden">
        <div className="grid grid-cols-4 gap-1 px-2">
          ...
          <button
            onClick={() => setActiveTab("metas")}
            className={`flex flex-col items-center gap-1 py-3 px-2 rounded-lg transition-all ${
              activeTab === "metas" ? "text-primary bg-primary/10 scale-105" : "text-muted-foreground active:scale-95"
            }`}
          >
